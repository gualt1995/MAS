...
import mas.dummyagents.DummyMovingAgent;
...
public class PrincipalD {
	...
	public static void main(String[] args){
		...
		startAgents(agentList);
			...
			if(PLATFORM_isMAIN){
				...
				containerList.putAll(createAndConnectContainer("Container_Gualtiero_Leon", PLATFORM_HOSTNAME, PLATFORM_ID, PLATFORM_PORT));
	...
	private static List<AgentController> createAgents(HashMap<String, ContainerController> containerList) {
		...
		c = containerList.get("Container_Gualtiero_Leon");
		Assert.assertNotNull("This container does not exist",c);
		agentName="Leon";
		String gatekeeperName="GK";
		try {
			Object[] objtab=new Object[]{gatekeeperName};//used to give informations to the agent
			AgentController	ag=c.createNewAgent(agentName,DummyMovingAgent.class.getName(),objtab);
			agentList.add(ag);
